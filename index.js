...
//   const container = document.getElementById('category-bar-container');
//   container.classList.add =`category-bar`
//   category.forEach(item => {
//       const div = document.createElement('div')
//       div.innerHTML = ` <button class='btn text-sm'>${item.category_name}</button>`
//       container.appendChild(div)
...
//         div.innerHTML = `<button onclick ="loadNewsByCategory('${category.category_id}')"></button>`
//     container.appendChild(div)
...
const displayNews = (cat) => {
  handleSearch()
  const newsContainer = document.getElementById("news-container");
  newsContainer.innerHTML=''
  cat.forEach((singleNews) => {
    const div = document.createElement("div");
    div.classList.add("singleNews");
...
const handleSearch =()=>{
  const searchNews = document.getElementById('search-box').value;
